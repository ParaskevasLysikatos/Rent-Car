...
$factory->define(Rental::class, function (Faker $faker) {
    ...
    $voucher        = rand(0, 5) * 5;
    $total_net      = Rental::calculateNetTotal($duration, $rate, $discount, $voucher, $transport_fee, $insurance_fee, $options_fee, $fuel_fee, $subcharges_fee);

    return [
        'status'            => Arr::random(Rental::getValidStatuses()),
        'duration'          => $duration,
        'rate'              => $rate,
...
$factory->afterMakingState(Rental::class, 'with_user', function ($booking, $faker) {
    $booking->user_id = factory(User::class)->state('with_role')->create()->id;
});

$factory->afterMakingState(Rental::class, 'with_contact_information', function ($booking, $faker) {
    $booking->contact_information_id = factory(ContactInformation::class)->create()->id;
});

$factory->afterMakingState(Rental::class, 'with_vehicle', function ($booking, $faker) {
...
$factory->afterMakingState(Rental::class, 'with_brand', function ($booking, $faker) {
    $booking->brand_id = factory(Brand::class)->state('with_profiles')->create()->id;;
});

$factory->afterMakingState(Rental::class, 'with_source', function ($booking, $faker) {
    $booking->source_id = factory(BookingSource::class)->create()->id;;
});

$factory->afterMakingState(Rental::class, 'with_agent', function ($booking, $faker) {
    $booking->agent_id = factory(Agent::class)->state('with_contact_information')->create()->id;;
});

$factory->afterMakingState(Rental::class, 'with_stations', function ($booking, $faker) {
...
$factory->afterCreatingState(Rental::class, 'with_documents', function ($booking, $faker) {
