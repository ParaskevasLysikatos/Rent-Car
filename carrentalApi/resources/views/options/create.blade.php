@extends('layouts.multiLingualCreate', [
    'formAction' => route('create_option', ['locale' => $lng ?? 'el', 'option_type' => $option_type]),
    'formCancel' => route('options', ['locale' => $lng ?? 'el', 'option_type' => $option_type]),
...
@section('main-fields')
    ...
    <div class="input-group mb-3">
        <input required type="text" class="form-control" id="code" name="code" @if(isset($option)) value="{{ $option->code }}"@endif>
    </div>

    <label for="order">{{__('Σειρά προτεραιότητας')}}*</label>
    <div class="input-group mb-3">
        <input required type="number" class="form-control" id="order" name="order" @if(isset($option)) value="{{ $option->order }}"@else{{ 0 }}@endif>
    </div>
