...
import { Component, ElementRef, EventEmitter, HostListener, Injector, OnDestroy, OnInit, Output, TemplateRef, ViewChild } from '@angular/core';
import { AbstractControl, FormArray, FormControl, FormGroup, FormGroupName, Validators } from '@angular/forms';
...
import { IInvoices } from 'src/app/invoices/invoices.interface';
...
import { PrintRentalService } from '../print-rental/print-rental.service';
...
import { VehicleExchangesService } from 'src/app/vehicle-exchanges/vehicle-exchanges.service';
import { IVehicleExchanges } from 'src/app/vehicle-exchanges/vehicle-exchanges.interface';
...
import { DriverEmpSelectorComponent } from 'src/app/_selectors/driverEmp-selector/driverEmp-selector.component';
...
export class RentalFormComponent extends AbstractFormComponent implements OnInit,OnDestroy {
  ...
  overall_travelled_km!: string;
  overallCurrentKm: number;

  afterDLbool: boolean = false;
  afterClosedStatusSave: boolean= false;
  ...
  form = this.fb.group({
    ...
    tags: [],//from method
    duration: [],//ok
    checkout_driver_id: [, Validators.required],//ok
    documents: [],//from method
    comments: [],//ok
    sequence_number: [],//initilized on its own
    modification_number: [],//initilized on its own
    status: [],//ok
    ...
    sub_account_id: [],//ok
    sub_account_type: [],//ok
    phone: [],//ok from driver
    drivers: [],//from method
    agent_id: [],//ok
    sub_account: [],//compact comes
    program_id: [],//ok
    ...
    checkout_km: [],//ok
    checkout_fuel_level: [],//ok

    booked_checkin_datetime: [],//ok
    checkin_datetime: [],//ok
    ...
    checkin_km: [],//ok
    checkin_fuel_level: [],//ok
    checkin_driver_id: [],//ok

    flight: [],//ok
    type_id: [], //ok
    vehicle_id: [, Validators.required],//ok
    extra_charges:[],//ok for invoices

    excess: [],//ok
    extra_day: [],//ok
    may_extend: [0,],//ok
    extension_rate: [],//ok
    payments: [[],],//from method
    invoices: [],//will see
  ...
  @ViewChild('checkoutDriver', { static: false }) checkout_driverRef: DriverEmpSelectorComponent;
  @ViewChild('checkinDriver', { static: false }) checkin_driverRef: DriverEmpSelectorComponent;

  @ViewChild('driver') driver!: DriverSelectorComponent;
  ...
  @ViewChild('invoicesTable') invoicesTable!: MatTable<any>;
  ...
  driverEmpStatic:IDriver[];//for close-pre and check in status
  ...
  constructor(protected injector: Injector, protected optionsSrv: OptionsService<IOptionsCollection>,
    ...
    public formDialogSrv: FormDialogService, protected programSrv: ProgramService<IProgram>,
    ...
    public printRentalSrv: PrintRentalService, public typesSrv: TypesService<ITypes>, public driverEmpSrv:DriverEmpService<IDriver>,
    public exchangeSrv: VehicleExchangesService<IVehicleExchanges>, public notificationSrv: NotificationService,
    public createRenSrv: CreateRentalModalService, private urlSrv: Router, public printCheckboxSrv: PrintCheckboxService
  ...
  ngOnInit(): void {
    ...
    this.combinedSrv.getRentals().subscribe((res) => {
      ...
      this.driver.selector.data = res.drivers;
      this.vehicle_Ref.selector.data = res.vehicles;

      if (this.form.controls.status.value == 'closed' || this.form.controls.status.value == 'pre-check' || this.form.controls.status.value == 'check-in') { //only in status checkin exists
        this.checkin_driverRef.selector.data = res.driversEmp;
      }
      this.driverEmpStatic = res.driversEmp;
      this.checkout_driverRef.selector.data = res.driversEmp;
    });
  }
  ...
  ngAfterViewInit() {
    this.form.controls.summary_charges.valueChanges.subscribe((res) => {
      if (this.customUrl == 'create') {
        this.form.controls.options.patchValue(this.sorting(this.nullifyOptionsId(this.form.get('summary_charges.items').value)));
    ...
    //validate km on checkin status
    this.form.controls.checkin_km.valueChanges.subscribe(() => {
      console.log('a');
      if (this.form.controls.status.value == 'closed' || this.form.controls.status.value == 'pre-check' || this.form.controls.status.value == 'check-in') {
        this.form.controls.checkin_km.addValidators(Validators.min(+this.form.controls.checkout_km.value));
        this.form.controls.checkin_km.updateValueAndValidity({ onlySelf: true, emitEvent: false });
      }
      else {
        this.form.controls.checkin_km.clearValidators();
        this.form.controls.checkin_km.updateValueAndValidity({ onlySelf: true, emitEvent: false });
      }
    });

    this.form.controls.driver_id.valueChanges.subscribe(() => {
       this.driverEvent();
    });

    this.printRentalSrv.afterDataLoadSubject.pipe(filter((value)=> value)).subscribe(res => {
       //after data load runs one time
        this.printRentalSrv.afterDataLoadSubject.next(false);
  ...
  */
        ...
        this.printRentalSrv.total_paidSubject.next(this.form.get('summary_charges.total_paid').value);//sent calculated payments
      ...
      if (+this.basicRentalEdit_Ref?.value == 0 && this.customUrl != 'create' || +this.basicRentalEdit_Ref.value == 0 && this.form.controls.convert.value == true) {
       ...
       //   console.log(this.items.at(this.findItem('Μίσθωμα Οχήματος')).get('total_cost').value);
...
*/
  ...
  ngOnDestroy() {
    ...
    this.printRentalSrv.total_paidSubject.next(0);
    ...
    //from convert booking
    this.createRenSrv.createRenSubject.next(null);
    this.createRenSrv.stationOutSub.next(null);
    this.createRenSrv.stationInSub.next(null);
    this.createRenSrv.driverSub.next(null);
    //this.createBookingSrv.vehicleSub.next(null);
    this.createRenSrv.sourceSub.next(null);
    this.createRenSrv.agentSub.next(null);
    this.createRenSrv.typeSub.next(null);
    this.createRenSrv.payersSub.next(null);
    this.createRenSrv.summaryChargesSub.next(null);

    //new modal
    this.selectorSrv.createNewObjDriver.next(false);
...
*/
    ...
    // automation for extension_rate
    else if (+item?.option?.id == 26) {// check extension_rate to match initial form values
      if (new Date() > new Date(this.form.controls.checkin_datetime.value)) {//this.form.controls.extension_rate.value
        item.quantity = 1;
        item.duration = this.form.controls.duration.value;
      ...
      //to calculate unsaved km
      let currentCheckout_km = +this.form.controls.checkout_km.value;
      let currentCheckin_km = +this.form.controls.checkin_km.value;

      let overallKm = +this.overall_travelled_km + (currentCheckin_km - currentCheckout_km);
      this.overallCurrentKm = +overallKm;
    ...
    if (item?.option?.option_type != 'rental_charges') {//rental items are calculated above
      if (item?.quantity) {// casual case
        ...
        if (item.daily) {
         ...
         // console.log('recalc duration row');
  ...
  calculatePaid(payer: string){
    if (this.form.get('payments')?.value) {
      var payments = this.form.get('payments')?.value.filter((payment: IPayment) => {
...
//---------payments-----------------------------------//
  ...
  addPayment(type: string): void {
    this.formDialogSrv.showDialog(CreatePaymentComponent, {
      typePayment: type, rental_id: this.form.controls.id.value, rental: true, payers: this.payers, comesFromNew: this.customUrl,
  ...
  editPayment(element: IPayment): void {
    ...
    this.formDialogSrv.showDialog(EditPaymentComponent, {
      object: payment, type: element.payment_type, rental: true, payers: this.payers, payerType: payerType,
       comesFromNew: 'create', excess: this.form.get('excess').value,discount_fee: this.form.get('summary_charges.discount_fee').value
    }, false)
      .subscribe((payment: any | undefined) => {
        if (payment) {
...
//---------invoices-----------------------------------//
  ...
  editInvoice(element: IInvoices): void {
    const invoices = this.form.get('invoices')?.value;
    const invoice: any = Object.assign({}, element);
    this.formDialogSrv.showDialog(EditInvoicesComponent, { object: invoice }, false)
      .subscribe((invoice: any | undefined) => {
        if (invoice) {
          const index = invoices.indexOf(element);
          invoices[index] = invoice;
          this.form.get('invoices')?.patchValue(invoices);
          this.renderPaymentTables();
  ...
  deleteInvoice(element: IInvoices): void {
    const invoices = this.form.get('invoices')?.value;
    const index = invoices.indexOf(element);
  ...
  addExchangeVehicle() {
    this.formDialogSrv.showDialog(CreateVehicleExchangesComponent, {
      rental_id: this.form.controls.id.value, //rental_id
      vehicle_id: this.form.controls.vehicle_id.value,
      checkout_km: this.form.controls.checkout_km.value,
      checkout_fuel_level: this.form.controls.checkout_fuel_level.value,
      old_vehicle_Create: this.vehicleData?.model + ' ' + this.vehicleData?.make,
      checkin_datetime:this.checkin_datetime,
      checkout_datetime: this.checkout_datetime,

    }, false)
      .subscribe((exchange: IVehicleExchanges) => {
        if (exchange) {
          this.exchangeSrv.create(exchange).subscribe((res) => { //saves directly to backend, not iconicly
            //this.form.patchValue(res.rental);
            this.exchanges.push(res);

            this.vehicle_id = res.rental?.vehicle_id;
            this.form.controls.vehicle_id.patchValue(res.rental?.vehicle_id);
            this.vehicleData = res.new_vehicle_source;

            this.type_id = res.rental?.type_id;
            this.form.controls.type_id.patchValue(res.new_vehicle_source.type_id);
            this.form.controls.excess.patchValue(res.rental?.excess);

            this.form.controls.checkout_km.patchValue(res.new_vehicle_rental_co_km);
            this.form.controls.checkout_fuel_level.patchValue(res.new_vehicle_rental_co_km);

            this.notificationSrv.showSuccessNotification('Επιτυχής αντικατάσταση');
          });
        }
      });
  }
  ...
  ShowCheckboxSignature(){
    this.formDialogSrv.showDialog(RentalSignatureComponent, {obj: this.form.value }, false)
      .subscribe(() => {
      });
  }
  //-------------status header---------------------//
  ...
  StatusChange(status: string) {
    if (status != 'check-in' && status != 'pre-check') {
      if (status == 'closed' && new Date() < new Date(String(this.checkin_datetime))) {
        this.notificationSrv.showErrorNotification('Δεν γίνεται να κλέισει η μίσθωση με μελλοντική Ημ/νία.');
        return;
      }

      if (status == 'closed' && this.form.controls.checkin_km.value == null || status == 'closed' && this.form.controls.checkin_fuel_level.value == null || status == 'closed' && this.form.controls.checkin_driver_id.value == null){
           this.notificationSrv.showErrorNotification('Δεν γίνεται να κλέισει η μίσθωση χωρίς στοιχεία checkin-km,checkin-fuel,checkin-driver');
           return;
         }
      }
      ...
      if (status == 'closed' && new Date() > new Date(String(this.checkin_datetime)) ){
        this.afterClosedStatusSave = true;
      }
    ...
    //this.createRenSrv.callSaveSubject.next(true);
    //disable Πληροφορίες Παραλαβής οχήματος
    if (status == 'closed' || status == 'pre-check' || status == 'check-in') {
      this.form.controls.checkin_km.enable();
      this.form.controls.checkin_fuel_level.enable();
      this.form.controls.checkin_driver_id.enable();
      this.checkin_driverRef.selector.data = this.driverEmpStatic;
    } else {
      this.form.controls.checkin_km.disable();
      this.form.controls.checkin_fuel_level.disable();
      this.form.controls.checkin_driver_id.disable();
    }
  }
  ...
  extraKmHandler(event: number) {
    this.form.get('summary_charges.distance_rate').patchValue(event);
    this.activateCalcs();
  }

  freeKmHandler(event: number) {
    this.form.get('summary_charges.distance').patchValue(event);
    this.activateCalcs();
  }
  ...
  driverData: any;
  driver_id: string;
  driverLicenceExp: string = '';
  driverComplete: boolean = false;

  driverEvent() {
   // this.driver.dataLoaded.subscribe(res => {
    this.selectorSrv.searchDriver.subscribe(res => {
      if (this.driverComplete && this.afterDLbool) {//after save to not run, comes from db, afterDlbool cause value changes runs twice
      this.driverLicenceExp = moment(res?.licence_expire).format('DD/MM/YYYY');
      this.LicenceExpCheck(res?.licence_expire, String(this.checkin_datetime));
      console.log(this.driverLicenceExp + '  aaaaaa');
      this.driverData = res;
  ...
  groupEvent() {
    this.selectorSrv.searchGroup.subscribe(res => {
      ...
      if (res?.id != this.vehicleData?.type_id) {//check current vehicle is of this group,otherwise nullify
        ...
        this.selectorSrv.searchVehicle.next(null);
        this.form.controls.checkout_km.patchValue(null);
        this.form.controls.checkout_fuel_level.patchValue(null);
      }
  ...
  vehicleInsuranceExp: string = '';
  vehicleKteoExp: string = '';
  ...
  vehicleEvent() {
    if (this.hasGroup) { //has group active
      this.selectorSrv.searchVehicle.subscribe(res => {
        this.vehicleInsuranceExp = moment(res?.insurance?.date_expiration).format('DD/MM/YYYY');
        this.vehicleKteoExp = moment(res?.KTEO?.date_expiration).format('DD/MM/YYYY');
        this.VehicleExpCheck(this.vehicleKteoExp,this.vehicleInsuranceExp,String(this.checkin_datetime));
        ...
        this.form.controls.checkout_km.patchValue(res?.km);//km
        this.form.controls.checkout_fuel_level.patchValue(res?.fuel_level);//fuel
  ...
  calcPlusday() {
    ...
    let timeOut = dateOut.getHours() * 3600 + dateOut.getMinutes() * 60;//seconds
    let timeIn = dateIn.getHours() * 3600 + dateIn.getMinutes() * 60;//seconds
    ...
    this.LicenceExpCheck(this.driverLicenceExp, String(this.checkin_datetime));
    this.VehicleExpCheck(this.vehicleKteoExp, this.vehicleInsuranceExp, String(this.checkin_datetime));
  ...
  calcPlusdayCheckbox() {
    ...
    this.daysDiff = dayDifference / (1000 * 3600 * 24);
    let days = this.diffDays2(dateOut, dateIn);
    console.log(this.durationInitial + ' < ' + this.daysDiff);
    if (this.durationInitial < this.daysDiff) {//dates range increased
  ...
  EventClick(event: Event) {
    ...
    // else if (this.driver_id != this.form.controls.driver_id.value) { // Οδηγός
  ...
  compareDates(dateToCheck:string,dateCheckIN: string):boolean{//a check date like driver licence exp, a limit date like checkin date
  // console.log(dateToCheck+ ' > '+ dateLimit);
    if (dateToCheck == null || dateToCheck == undefined || dateToCheck ==''){
    return false;//missing licenceExp or kteo
  }
    if (new Date(dateToCheck) < new Date(dateCheckIN)){// chech in must be less than exp, add a day
      // this.notificationSrv.showErrorNotification('date was less');
      return true;
    }else{
// this.notificationSrv.showSuccessNotification('date exp exceeded');
      return false;
    }
  }

  LicenceExpCheck(licenceExpDate:string,checkinDate:string){
    let checkLE = this.compareDates(licenceExpDate, checkinDate);
    if (checkLE) {
     // this.form.controls.driver_id.setErrors({ licence_expired: true });
     // this.notificationSrv.showErrorNotification('will be licence expired until: ' + moment(checkinDate).format('YYYY-MM-DD'));
    } else {
     // this.form.controls.driver_id.setErrors(null);
    }
  }

  VehicleExpCheck(KTEOExpDate: string,InsurDate:string ,checkinDate: string) {//check kteo and insurance of vehicle
    let checKTEO = this.compareDates(KTEOExpDate, checkinDate);
    let checkInsurance = this.compareDates(InsurDate, checkinDate);
    if (checKTEO) {
    //  this.form.controls.vehicle_id.setErrors({ kteo_expired: true });
     // this.notificationSrv.showErrorNotification('will be kteo expired until: ' + moment(checkinDate).format('YYYY-MM-DD'));
    } else {
    //  this.form.controls.vehicle_id.setErrors(null);
    }
    //-------//
    if (checkInsurance) {
     // this.form.controls.vehicle_id.setErrors({ insurance_expired: true });
     // this.notificationSrv.showErrorNotification('will be insurance expired until: ' + moment(checkinDate).format('YYYY-MM-DD'));
    } else {
     // this.form.controls.vehicle_id.setErrors(null);
    }
  }
  ...
  sourceEventInit(id:string) {
    ...
    this.sourceSrv.edit(id).subscribe(res => {
      ...
      if (!this.sourceComplete && this.createRenSrv.createRenSubject.getValue() == null && this.customUrl == 'create') { // after save to not run,db has them.and convert, but create pass
        this.source_id = res?.id;
        this.form.controls.brand_id.patchValue(res?.brand_id);// based on source, brand is selected but can be altered by user
        this.form.controls.source_id.patchValue(res?.id)
        this.form.controls.program_id.patchValue(res?.program_id);
  ...
  driverEventInit(id: string) {
    // this.driver.dataLoaded.subscribe(res => {
    this.driverSrv.edit(id).subscribe(res => {
      this.driverLicenceExp = moment(res?.licence_expire).format('DD/MM/YYYY');
      this.LicenceExpCheck(res?.licence_expire, String(this.checkin_datetime));
      console.log(this.driverLicenceExp + '  aaaaaa');
      this.driverData = res;
     // this.driver_id = res?.id;
      this.selectorSrv.searchDriver.next(res);
      this.form.controls.phone.patchValue(this.driverData?.contact?.phone);
      this.driverComplete = true;
    });
  }
